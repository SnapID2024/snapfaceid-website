...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BACKEND_API_URL}/api/admin/geofence`, {
      method: 'GET',
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.detail || 'Failed to fetch geofence settings' },
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.detail || 'Failed to save geofence settings' },
        { status: response.status }
      );
    }
